...
@HtmlImport("frontend://frontend://src/components/styles_scheduler.html")
public class CalendarioView extends Div {
    ...
    private void createToolbar() {
        ...
        Button buttonHeight = new Button("Calendar height", event -> new CalendarioView.HeightDialog().open());
...
//                "}");
        ...
        calendar.addEntryClickedListener(event -> new CalendarioView.DemoDialog(calendar, event.getEntry(), false).open());
        ...
        calendar.addTimeslotsSelectedListener(event -> {
            ...
            entry.setColor("dodgerblue");
            new CalendarioView.DemoDialog(calendar, entry, true).open();
        });
        ...
        calendar.addLimitedEntriesClickedListener(event -> {
            ...
            if (!entries.isEmpty()) {
                ...
                entries.stream()
                        ...
                        .map(entry -> {
                            NativeButton button = new NativeButton(entry.getTitle(), clickEvent -> new CalendarioView.DemoDialog(calendar, entry, false).open());
